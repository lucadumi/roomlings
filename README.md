...
- Refined low-poly materials and cabinet details, soft filtered sunlight shadows, and shared contact-shadow masks that ground the furniture without a post-processing pass. Shadow maps are reused for camera-only changes and refreshed when objects move.
- Frame-rate-independent camera and object motion, continuous steam, and coordinated daylight/evening transitions. Reduced-motion rooms stop idle drawing and still update for controls, ledger changes and the clock.
- A clear WebGL fallback that keeps the complete shared ledger usable.
