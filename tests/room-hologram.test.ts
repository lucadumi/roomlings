...
test('placement cameras share the entry scale instead of candidate closeups without locking manual zoom', () => {
  for (const file of ['KitchenWorld.tsx', 'ChoreRoomWorld.tsx']) {
    ...
    assert.match(source, /roomCameraZoom\(latest\.overviewFocus \? 1 : currentControls\.zoom, closeRoom, 'kitchen',\s*roomFocused \? latest\.roomZoom : 1\)|config\.cameraZoom\(displayedZoom, closeRoom, config\.roomId\)/)
  ...
  const kitchen = readFileSync(new URL('../src/KitchenWorld.tsx', import.meta.url), 'utf8')
  assert.match(kitchen, /const roomFocused = latest\.overviewFocus \|\| currentControls\.roomView \|\| \(focus === 'room' && !selectedActor\)/)
