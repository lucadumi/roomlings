...
test('camera movement copy also describes zooming out', { tag: '@room' }, async ({ page, populatedHousehold: _household }) => {
  ...
  const entrySpan = Number(await canvas.getAttribute('data-camera-span'))
  expect(entrySpan).toBeGreaterThan(0)
  await expect(readout).toHaveText('100%')
  await page.getByRole('button', { name: 'Close the fridge', exact: true }).click()
  await page.clock.runFor(32)
  await expect(world).toHaveAttribute('data-focus', 'fridge')
  await expect(world).toHaveAttribute('data-camera-moving', 'false')
  const focusedSpan = Number(await canvas.getAttribute('data-camera-span'))
  await expect(readout).toHaveText(`${Math.round(entrySpan / focusedSpan * 100)}%`)
  await page.emulateMedia({ reducedMotion: 'no-preference' })
  await page.getByRole('button', { name: 'Zoom out', exact: true }).click()
  // A slow real frame can finish the transition; hold one animation frame for the status assertion.
  await page.clock.runFor(16)
  const movingSpan = Number(await canvas.getAttribute('data-camera-span'))
  expect(movingSpan).toBeGreaterThan(focusedSpan)
  await expect(readout).toHaveText(`${Math.round(entrySpan / movingSpan * 100)}%`)
  ...
  await expect(world).toHaveAttribute('data-camera-moving', 'false')
  await expect(canvas).toHaveAttribute('data-camera-zoom', '0.90000')
  expect(Number(await canvas.getAttribute('data-camera-span'))).toBeCloseTo(focusedSpan / 0.9, 4)
  await expect(readout).toHaveText(`${Math.round(entrySpan / focusedSpan * 0.9 * 100)}%`)
