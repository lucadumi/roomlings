...
test('bathroom objects open room-specific chores and the shared supply list', { tag: '@room' }, async ({ page, populatedHousehold: _household }) => {
  ...
  for (const area of ['sink', 'mirror', 'toilet', 'bath', 'floor']) {
    ...
    if (area === 'floor') await expect(page.locator('.bathroom-world')).toHaveAttribute('data-focus', area)
    else await expect(page.getByRole('combobox', { name: 'Chore object', exact: true })).toHaveAttribute('data-value', `default-bathroom-${area}`)
  ...
  await expect(page.getByRole('combobox', { name: 'Chore object', exact: true })).toHaveAttribute('data-value', 'default-bathroom-supply-shelf')
  await page.getByRole('button', { name: 'Close panel', exact: true }).click()
